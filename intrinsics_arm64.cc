...
//TI means taint integral type,including int, long, char, short.
static void CreateTIIntToVoidLocations(ArenaAllocator* arena, HInvoke* invoke) {
     LocationSummary* locations = new (arena) LocationSummary(invoke, LocationSummary::kNoCall, kIntrinsified);
	 locations->SetInAt(0, Location::RequiresRegister());//TODO need to find the register mapped to template type
	 locations->SetInAt(1, Location::RequiresRegister());
}

//TF means taint floating point type,including float,double.
static void CreateTFIntToVoidLocations(ArenaAllocator* arena, HInvoke* invoke) {
	LocationSummary* locations = new (arena) LocationSummary(invoke, LocationSummary::kNoCall, kIntrinsified);
...
//GetTaint
void IntrinsicCodeGeneratorARM64::VisitTaintGetTaint(HInvoke* invoke){
}
void IntrinsicLocationsBuilderARM64::VisitTaintGetTaint(HInvoke* invoke) {
     CreateTptIntToVoidLocations(arena_, invoke);
}
